...
class BtouchVideomatrix extends utils.Adapter {
	...
	async createStates() {
		...
		*/
		...
		var tmpOff = await this.getStateAsync('Labels.output_' + (0).toString().padStart(2, '0'));
		outputNames[0] = tmpOff.val;
	...
	matrixChanged(id, val, ack) {
		...
		if (id.toString().includes('SelectBool.input_')) {
			...
			if (ack == false) {	//Aenderung per GUI
				...
				let tmpIn = id.substring(iStart + 1, id.indexOf('_'));
				let tmpOut = id.substring(id.lastIndexOf(' '));
